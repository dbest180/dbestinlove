...
5. **The agent environment has Ruby, Jekyll, a headless browser, and ImageMagick — all installed system-wide, none of it in the repo.** Build and preview with the repo's own documented commands (`bundle install`, then `bundle exec jekyll serve --baseurl ""`); both work with no env vars or workarounds. Full setup and the rebuild recipe are in `DEEPSEEK_REQUIREMENTS.md`. You can also measure or screenshot the deployed page at any width. **Caveat:** the agent cannot *view* screenshots — the current model declares no image input. Visual judgement still routes through a human; DOM measurement is the substitute, and it is how the hero-crop question in §2 was settled. Move sessions to a vision model and that limitation disappears.
