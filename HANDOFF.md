...
| `.gitignore` (P3) | Added: `_site/`, `.jekyll-cache/`, `.sass-cache/`, and `.tools/` (the agent's workspace-local Ruby gems + Chromium — see `DEEPSEEK_REQUIREMENTS.md`). |
...
5. **The agent environment now has Ruby, Jekyll, *and* a headless browser** — all installed workspace-locally under `.tools/` (gitignored); setup is in `DEEPSEEK_REQUIREMENTS.md` §1. The loop is no longer push-and-pray: build locally with `jekyll build` (requires `JEKYLL_NO_BUNDLER_REQUIRE=1`, or Jekyll hands off to Bundler and dies on missing default gems), and measure or screenshot the deployed page directly. **Caveat:** the agent cannot *view* screenshots — the current model declares no image input. Visual judgement still routes through a human; DOM measurement is the substitute, and it is how the hero-crop question above was settled. Move sessions to a vision model and that limitation disappears.
